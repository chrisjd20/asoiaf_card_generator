...
def BuildAttachCardFactionWithData(AttachData, units_folder, attachments_folder, graphics_folder, tactics_folder, AsoiafFonts, AsoiafData, lang, AsoiafDataTranslations):
        ...
        for index in range(len(all_abilities)):
            ...
            if len(skillability_icon_images) > 0:
                if len(skillability_icon_images) == 1:
                    ...
                    #if AttachData['Name'].upper() == "PYROMANCERS":
                    #    pdb.set_trace()
                    if index < len(all_abilities)-1:
                        yAbilityOffset += addDivider(SkillBarsOffset - 52, yAbilityOffset)
    ...
    text_draw.text((0, 0), AttachData['Version'], font=VersionFont, fill="white")
...
def main():
    ...
    #root = tk.Tk()
    #app = ImageEditor(root, attach_card)
    #root.mainloop()
